...
const InterviewRoom: React.FC = () => {
  ...
  useEffect(() => {
    // Only process if meeting is active, AI is connected, and AI is not currently speaking
    if (!isMeetingStarted || !gemini.isConnected || gemini.isAISpeaking || gemini.isProcessing || isProcessingUserInput) {
      return;
    }
    ...
    // Check if user has stopped recording (finished speaking)
    if (!speech.isRecording && currentTranscript && currentTranscript !== lastProcessedTranscript) {
      const lastMessage = currentTranscript.split('\n').pop();
      
      if (lastMessage && lastMessage.trim()) {
        const cleanMessage = lastMessage.replace(/\[\d+:\d+:\d+\s*[APM]*\]\s*/, '').trim();
        
        // Only send if it's a new, substantial message (more than 3 characters)
        if (cleanMessage.length > 3) {
          setIsProcessingUserInput(true);
          setLastProcessedTranscript(currentTranscript);
          
          console.log('📤 Sending to AI:', cleanMessage);
          
          // Send to AI with a slight delay to ensure user is done speaking
          setTimeout(() => {
            gemini.sendMessage(cleanMessage).finally(() => {
              setIsProcessingUserInput(false);
            });
          }, 800); // Reduced from potential longer delay
        }
      }
    }
  }, [speech.isRecording, speech.transcript, isMeetingStarted, gemini.isConnected, gemini.isAISpeaking, gemini.isProcessing, lastProcessedTranscript, isProcessingUserInput]);

  // Reset conversation state when AI finishes speaking (enable user to speak again)
  useEffect(() => {
    if (!gemini.isAISpeaking && !gemini.isProcessing && !isProcessingUserInput && isMeetingStarted) {
      // Automatically restart speech recognition if it's not already running
      setTimeout(() => {
        if (!speech.isRecording) {
          console.log('🎤 Restarting speech recognition for continued conversation');
          speech.startRecording().catch(console.error);
        }
      }, 1000); // Wait 1 second after AI stops speaking
    }
  }, [gemini.isAISpeaking, gemini.isProcessing, isProcessingUserInput, isMeetingStarted, speech.isRecording]);
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className={`px-3 py-1 rounded-full text-sm ${
              gemini.isConnected ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300'
            }`}>
              {gemini.isConnected ? '🤖 AI Connected' : '🤖 AI Disconnected'}
            </div>
            <div className={`px-3 py-1 rounded-full text-sm ${
              stream.isCallActive ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300'
            }`}>
        ...
        <div className="flex-1 flex flex-col">
                      ...
                      <div className="mt-4">
                        ...
                        {gemini.isAISpeaking && (
                          <div className="bg-blue-900/30 border border-blue-500/30 rounded-lg p-4">
                            ...
                            {gemini.aiResponse && (
                              <p className="text-sm text-gray-300 mt-2">{gemini.aiResponse}</p>
                            )}
                          </div>
                        )}
                        
                        {!gemini.isAISpeaking && !gemini.isProcessing && !isProcessingUserInput && (
          ...
          <div className="p-6 bg-gray-800 border-t border-gray-700">
            <div className="flex items-center justify-center gap-4">
              ...
              >
                {speech.isRecording ? <MicOff className="w-5 h-5" /> : <Mic className="w-5 h-5" />}
            ...
            {(speech.isRecording && !gemini.isAISpeaking && !gemini.isProcessing) && (
          ...
          <div className="flex-1 p-4 overflow-y-auto">
            <div className="space-y-3">
              {speech.transcript.split('\n').filter(line => line.trim()).map((line, index) => (
                <div key={index} className="bg-blue-900/20 border-l-4 border-blue-500 p-3 rounded">
              ...
              {gemini.aiResponse && (
                <div className="bg-green-900/20 border-l-4 border-green-500 p-3 rounded">
                  <div className="text-xs text-green-300 mb-1">AI Response:</div>
