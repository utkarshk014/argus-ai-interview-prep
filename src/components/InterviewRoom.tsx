...
const InterviewRoom: React.FC = () => {
  ...
  // ONLY FIX: Stop microphone when AI is speaking to prevent feedback
  useEffect(() => {
  ...
  // ONLY FIX: Resume microphone when AI stops speaking
  useEffect(() => {
    if (!gemini.isAISpeaking && isMeetingStarted && !speech.isRecording) {
      console.log("🎤 Resuming microphone - AI finished speaking");
      setTimeout(() => {
        speech.startRecording().catch(console.error);
      }, 500); // Small delay to ensure AI audio stops
    }
  }, [gemini.isAISpeaking, isMeetingStarted]);

  // Original logic for processing user input (unchanged)
  useEffect(() => {
    if (
      !speech.isRecording &&
      speech.transcript &&
